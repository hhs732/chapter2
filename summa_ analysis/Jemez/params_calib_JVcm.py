...
def hru_ix_ID(p1, p2, p3, p4, p5, p6, p7, p8, p9):#, p10, p11, p12, p13, p14, p15, p16, p17, p18, p19, p20, p21
    ...
    c = list(itertools.product(ix1,ix2,ix3,ix4,ix5,ix6,ix7,ix8,ix9))#,ix10,ix11,ix12,ix13,ix14,ix13,ix14,ix15,ix16,ix17,ix18,ix19,ix20,ix21
    ix_numlist=[]
    for tup in c:
...
def param_fill(p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, p11, p12, p13, p14, p15, p16, p17, p18): #, p19, p20
    ...
    p1l =[]; p2l =[]; p3l =[]; p4l=[]; p5l =[]; p6l =[]; p7l =[]; p8l =[]; p9l =[]; p10l=[]; p11l =[]; p12l =[]; 
    p13l =[]; p14l=[]; p15l = []; p16l =[]; p17l=[]; p18l = []#; p19l = []; p20l = []
    for tup in b:
...
pt = Dataset("C:\Users\HHS\summaTestCases_2.x\settings\Jemez\summa_zParamTrial_variableDecayRate.nc")
...
print pt.variables['hruIndex'][:]
#%% #create new paramtrail.nc file and adding vaiables to it --- summa_zParamTrial_variableDecayRate_test
paramfile = Dataset("summa_zParamTrial_JmzVcm_open_calib.nc",'w',format='NETCDF3_CLASSIC') #create new paramtrail.nc file

hru = paramfile.createDimension('hru', None)
hidx = paramfile.createVariable('hruIndex', np.float64,('hru',)) # add hruIndex variable
 
param_nam_list = ['mw_exp','z0Snow','albedoMax','albedoMaxVisible','albedoMinVisible',
                  'albedoMaxNearIR','albedoMinNearIR','albedoDecayRate','albedoRefresh','albedoSootLoad',
                  'rootingDepth','winterSAI','summerLAI','LAIMIN','LAIMAX','heightCanopyTop','heightCanopyBottom','z0Canopy'] 
...
#%% 
varcheck = Dataset ('summa_zParamTrial_JmzVcm_open_calib.nc')
print varcheck.variables['albedoSootLoad'][:]
print varcheck.variables['heightCanopyBottom'][:]
#%% # local attributes file
local_atrbt = Dataset("summa_zLocalAttributes_JmzVcm_open.nc",'w',format='NETCDF3_CLASSIC')
# define dimensions 
...
mHeight_sc = np.array([21]) #mHeight ftp://ftp.fluxdata.org/.ameriflux_downloads/measurement_height/BASE_MeasurementHeight_20180813.csv
...
local_atrbt.close()
#%%
lacheck = Dataset('summa_zLocalAttributes_JmzVcm_open.nc')
...
in_condi = Dataset("summa_zInitialCond_JmzVcm_open.nc",'w',format='NETCDF3_CLASSIC')
...
in_condi.close()
#%%
iccheck = Dataset("summa_zInitialCond_JmzVcm_open.nc")
