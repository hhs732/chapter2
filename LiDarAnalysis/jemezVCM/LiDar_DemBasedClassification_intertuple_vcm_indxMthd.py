...
def classificationNorthnessElev(index_ls,elev1,elev2):#0.25
...
topo_dimension_vcm = classificationNorthnessElev(indexVcm_cut.values,2830,3140)

#%% classification with grountpoints from veg dem file :VCM
upGroundPointsVegVcm = calculatingUpGroundPoints (coordsVegVcm_intg,dem_groundPointsVegVcm_intg,101)
#classification of snow las file with veg_grountpoints
upGroundPointsSnwVegVcm = calculatingUpGroundPoints (coordsSnwVcm_intg,dem_groundPointsVegVcm_intg,101)

path2npoutVeg="./upGroundPointsVegVcm.npy"
np.save(path2npoutVeg,upGroundPointsVegVcm)   
upLoadUpGroundPointsVeg=np.load(path2npoutVeg)

path2npoutSnw="./upGroundPointsSnwVegVcm.npy"
...
vegClassVcm_indx, vegClassVcm_df = createClassIndexBasedDF(vegClassVcm,101)
...
#allTreeClassNoLowVegVcm_df.set_index(np.arange(0,len(allTreeClassNoLowVegVcm_df)))

#all 0pen classification based on veg dem
all0penClassVcm_indx = (pd.concat([allTreeReturnVcm_indx,pd.DataFrame(vegClassVcm_indx)])).drop_duplicates(keep=False, inplace=False)
...
all0penClassNoGrass_df2 = extractingDFwithNO0O0OTEqualXYtoSmallDF0fromBigDF(all0penClassVcm,lowVeg0penClassVcm)
concLow0penCls = pd.concat([lowVeg0penClassVcm3c,all0penClassVcm])
all0penClassNoGrass_df = concLow0penCls.drop_duplicates(keep=False, inplace=False)
...
snowClassVcm_indx_DF = np.vstack([snowClassVcm_df['x']-np.min(vegClassVcm_df['x']),
                                  snowClassVcm_df['y']-np.min(vegClassVcm_df['y']),
                                  snowClassVcm_df['z']]).T
snowClassVcm_indx = snowClassVcm_indx_DF[:,1]+1001*snowClassVcm_indx_DF[:,0]
snowClassVcm_df.index = snowClassVcm_indx.astype(int)
...
allNoLowVegClassVcm_indx_DF = np.vstack([allTreeClassNoLowVegVcm_df['x']-np.min(vegClassVcm_df['x']),
                                         allTreeClassNoLowVegVcm_df['y']-np.min(vegClassVcm_df['y']),
                                         allTreeClassNoLowVegVcm_df['z']]).T
allNoLowVegClassVcm_indx = pd.DataFrame((allNoLowVegClassVcm_indx_DF[:,1]+1001*allNoLowVegClassVcm_indx_DF[:,0]).astype(int)).drop_duplicates(keep="first", inplace=False)
allSnowTreeNoLowVegClassVcm_df = snowClassVcm_df.loc[allSnowNoLowVegClassVcm_indx[0]]

#finding pixels that have trees
#allTreeClassVcm_df = extractingDFwithEqualXYtoSmallDF0fromBigDF(vegClassVcm_df,allTreeReturnVcm_df)    
allTreeClassVcm_df = vegClassVcm_df.loc[allTreeReturnVcm_indx[0]]
# snow under canopy
