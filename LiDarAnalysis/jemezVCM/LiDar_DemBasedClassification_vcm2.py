...
UnTree_sheltered_midElev_vcm = []
for row in allsnowUnderTreeClassVcm_df.itertuples():
    indxUT_shLe = np.where(((sheltered_midElev_vcm_df['x']==row.x) & (sheltered_midElev_vcm_df['y']==row.y)))[0]#
    if (np.size(indxUT_shLe))>0:
        UnTree_sheltered_midElev_vcm.append([row.x,row.y,row.z])# 
UnTree_sheltered_midElev_vcm_df = pd.DataFrame(UnTree_sheltered_midElev_vcm,columns=['x','y','z']) 

UnTree_neutral_midElev_vcm = []
for row in allsnowUnderTreeClassVcm_df.itertuples():
    indxUT_ntLe = np.where(((nuetral_midElev_vcm_df['x']==row.x) & (nuetral_midElev_vcm_df['y']==row.y)))[0]#
    if (np.size(indxUT_ntLe))>0:
        UnTree_neutral_midElev_vcm.append([row.x,row.y,row.z])# 
UnTree_neutral_midElev_vcm_df = pd.DataFrame(UnTree_neutral_midElev_vcm,columns=['x','y','z']) 
        
UnTree_exposed_midElev_vcm = []
for row in allsnowUnderTreeClassVcm_df.itertuples():
    indxUT_exLe = np.where(((exposed_midElev_vcm_df['x']==row.x) & (exposed_midElev_vcm_df['y']==row.y)))[0]#
    if (np.size(indxUT_exLe))>0:
        UnTree_exposed_midElev_vcm.append([row.x,row.y,row.z])# 
UnTree_exposed_midElev_vcm_df = pd.DataFrame(UnTree_exposed_midElev_vcm,columns=['x','y','z']) 
        
open_sheltered_midElev_vcm = []
for row in allSnow0penGroundClassVcm_df.itertuples():
    indxUT_shLe = np.where(((sheltered_midElev_vcm_df['x']==row.x) & (sheltered_midElev_vcm_df['y']==row.y)))[0]#
    if (np.size(indxUT_shLe))>0:
        open_sheltered_midElev_vcm.append([row.x,row.y,row.z])# 
open_sheltered_midElev_vcm_df = pd.DataFrame(open_sheltered_midElev_vcm,columns=['x','y','z']) 

open_neutral_midElev_vcm = []
for row in allSnow0penGroundClassVcm_df.itertuples():
    indxUT_ntLe = np.where(((nuetral_midElev_vcm_df['x']==row.x) & (nuetral_midElev_vcm_df['y']==row.y)))[0]#
    if (np.size(indxUT_ntLe))>0:
        open_neutral_midElev_vcm.append([row.x,row.y,row.z])# 
open_neutral_midElev_vcm_df = pd.DataFrame(open_neutral_midElev_vcm,columns=['x','y','z']) 
       
open_exposed_midElev_vcm = []
for row in allSnow0penGroundClassVcm_df.itertuples():
    indxUT_exLe = np.where(((exposed_midElev_vcm_df['x']==row.x) & (exposed_midElev_vcm_df['y']==row.y)))[0]#
    if (np.size(indxUT_exLe))>0:
        open_exposed_midElev_vcm.append([row.x,row.y,row.z])# 
open_exposed_midElev_vcm_df = pd.DataFrame(open_exposed_midElev_vcm,columns=['x','y','z']) 
