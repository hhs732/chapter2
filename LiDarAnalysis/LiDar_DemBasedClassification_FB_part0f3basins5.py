...
infileVegRoadVcm = ls.file.File("W:/_users/hsafa/Chapter2_snow_forest/LIDAR_analysis/JemezVcm/snow0ffRoad2pointsVCM.las", mode="r")
coordsVegRoadVcm = np.vstack((infileVegRoadVcm.x, infileVegRoadVcm.y, infileVegRoadVcm.z)).T
coordsVegRoadVcm_df = pd.DataFrame(coordsVegRoadVcm,columns=['x','y','z'])
coordsVegRoadVcm_df.sort_values(by=['x','y'],inplace=True)

infileSnwRoadVcm = ls.file.File("W:/_users/hsafa/Chapter2_snow_forest/LIDAR_analysis/JemezVcm/snow0nRoad2pointsVCM.las", mode="r")
coordsSnwRoadVcm = np.vstack((infileSnwRoadVcm.x, infileSnwRoadVcm.y, infileSnwRoadVcm.z)).T
coordsSnwRoadVcm_df = pd.DataFrame(coordsSnwRoadVcm,columns=['x','y','z'])
coordsSnwRoadVcm_df.sort_values(by=['x','y'],inplace=True)

for row in coordsSnwRoadVcm_df.itertuples():
    indexFinl = np.where(((coordsVegRoadVcm_df['x']==row.x)))[0]# & (dem_groundPointsVegVcm_intg['y']==row.y)
    print indexFinl
...
for row in coordsSnwVcm_intg.itertuples():
    indexVeg = np.where(((dem_groundPointsVegVcm_intg['x']==row.x) & (dem_groundPointsVegVcm_intg['y']==row.y)))[0]#
    if (np.size(indexVeg))>0:
        upG = row.z-dem_groundPointsVegVcm_intg['z'][indexVeg[0]]
...
# snow under canopy
allsnowUnderTreeClassVcm_df0 = pd.DataFrame(defineSpecificClassLess (allTreeSnowNoLowVegClassVcm_df, 2))
...
allSnow0penGroundClassVcm_df0 = pd.DataFrame(allSnowPixonGroundClassVcm,columns=['x','y','z']) 
allSnow0penGroundClassVcm_df = pd.DataFrame(defineSpecificClassLess(allSnow0penGroundClassVcm_df0,2),columns=['index','x','y','z']) #excluding returns with z more than 2m (conflicting pixels)

#I am working, please do not log out
#no snow on the ground
allNosnowClassVcm = pd.DataFrame(defineSpecificClassLess (allSnow0penGroundClassVcm_df0, 0.15))

#%% box plot
d1 = [allSnow0penGroundClassVcm_df['z'],allsnowUnderTreeClassVcm_df['z']]

fig, ax = plt.subplots(1,1, figsize=(20,15))
bp1 = ax.boxplot(d1, patch_artist=True)
bp1['boxes'][0].set(color='orange', linewidth=2, facecolor = 'skyblue', hatch = '/')
bp1['boxes'][1].set(color='darkseagreen', linewidth=2, facecolor = 'olive', hatch = '/')

plt.title('snow depth in Jemez_VCM; under canopy vs. open',fontsize=40)

plt.xticks([1,2], ['0pen sites','under canopy'], fontsize=30, rotation=0)#'open2016','open2017',
plt.yticks(fontsize=30)
#plt.xlabel('vegSc_year', fontsize=30)
plt.ylabel('snow depth (m)', fontsize=30)
plt.savefig('snowDepth_JemezVCM.png')
#%% ploting
