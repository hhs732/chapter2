...
#percentages = (np.random.randint(5,20, (len(people), segments)))
y_values = np.arange(1,len(feature_importances_error_0putBin_jmz[0][0].columns)+1)
color_jmz = ['khaki','yellowgreen','forestgreen','darkolivegreen']#rosybrown #indianred
plt.subplots(figsize=(20,15))
lft = 0
for imp, clr in zip(importance_bins_jmz,color_jmz):
    plt.barh(y_values,imp,left = lft,facecolor = clr)#,orientation = 'horizontal', 
    lft += imp
# Tick labels for x axis
feature_list1 = feature_importances_error_0putBin_jmz[0][0].columns
plt.xticks(fontsize=35) #rotation='vertical', 
plt.yticks(y_values, feature_list1, fontsize=35)

plt.ylabel('Features', fontsize=40); plt.xlabel('Importance', fontsize=40) 

meanAbsError = [feature_importances_error_0putBin_jmz[i][1] for i in range (4)]
fSCA_bin=['<0.3','0.3-0.55','0.55-0.8','>0.8']
legend = ['fSCA_bin {}; MAE = {}'.format(fSCA_bin[i],meanAbsError[i]) for i in range (4)]
plt.legend(legend,fontsize = 24)#, loc = 'upper center'
plt.title('Importance of features in 4 fSCA bins in JMZ', fontsize=40)#, position=(0.04, 0.88), ha='left'

plt.savefig('C:/1UNRuniversityFolder/Dissertation/Chapter 2-snow-forest/LiDarAnalysis/randomForest/JMZ/importance_0put_JMZ_bins_bar.png')
