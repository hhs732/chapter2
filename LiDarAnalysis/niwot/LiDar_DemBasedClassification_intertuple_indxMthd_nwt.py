...
def classificationNorthnessElev(index_ls,elev1,elev2,ncol,minX,minY):#0.25
    index_ls[:,4][(index_ls[:,2]<-0.2)&(index_ls[:,3]<elev1)]=11
    index_ls[:,4][(index_ls[:,2]>0.25)&(index_ls[:,3]<elev1)]=31
    index_ls[:,4][(index_ls[:,2]<=0.25)&(index_ls[:,2]>=-0.2)&(index_ls[:,3]<elev1)]=21
    index_ls[:,4][(index_ls[:,2]<-0.2)&(index_ls[:,3]>elev2)]=13
    index_ls[:,4][(index_ls[:,2]>0.25)&(index_ls[:,3]>elev2)]=33
    index_ls[:,4][(index_ls[:,2]<=0.25)&(index_ls[:,2]>=-0.2)&(index_ls[:,3]>elev2)]=23
    index_ls[:,4][(index_ls[:,2]<-0.2)&(index_ls[:,3]>=elev1)&(index_ls[:,3]<=elev2)]=12
    index_ls[:,4][(index_ls[:,2]>0.25)&(index_ls[:,3]>=elev1)&(index_ls[:,3]<=elev2)]=32
    index_ls[:,4][(index_ls[:,2]<=0.25)&(index_ls[:,2]>=-0.2)&(index_ls[:,3]>=elev1)&(index_ls[:,3]<=elev2)]=22
...
filenameS0fvcm = "C:/1UNRuniversityFolder/Dissertation/Chapter 2-snow-forest/LiDarAnalysis/niwot/snw0ff_nwt_dem.tif"
...
lasFilePath_vegVcm = "snw0ff_nwt_points1.las"
...
coordsSnwVcm_df['z'] = coordsSnwVcm_df['z']-0.081
...
path2npoutVeg="F:/hhs/lidar_analysis/upGroundPointsVegVcm.npy"
np.save(path2npoutVeg,upGroundPointsVegVcm)   
upLoadUpGroundPointsVeg=np.load(path2npoutVeg)

path2npoutSnw="F:/hhs/lidar_analysis/upGroundPointsSnwVegVcm.npy"
...
#%% box plot
d1 = [Open_el_vcm_df['z'],UnTree_el_vcm_df['z'],Open_nl_vcm_df['z'],UnTree_nl_vcm_df['z'],
      Open_sl_vcm_df['z'],UnTree_sl_vcm_df['z'],Open_em_vcm_df['z'],UnTree_em_vcm_df['z'],
      Open_nm_vcm_df['z'],UnTree_nm_vcm_df['z'],Open_sm_vcm_df['z'],UnTree_sm_vcm_df['z'],
      Open_eh_vcm_df['z'],UnTree_eh_vcm_df['z'],Open_nh_vcm_df['z'],UnTree_nh_vcm_df['z'],
      Open_sh_vcm_df['z'],UnTree_sh_vcm_df['z']]
...
plt.savefig('F:/hhs/lidar_analysis/snowDepth_JemezVCM_elev_topo.png')
#%% ploting
