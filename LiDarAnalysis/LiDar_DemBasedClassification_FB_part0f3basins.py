...
#%%
extentNR1=[minXNr1, maxXNr1, minYNr1, maxYNr1]
groundSnowClassNr1_ls = []
for clnr in groundSnowClassNr1:
    for rtnr in clnr:
        groundSnowClassNr1_ls.append(rtnr[0])
groundSnowClassNr1_df = pd.DataFrame(groundSnowClassNr1_ls)
#lonNr1, latNr1 = np.meshgrid(groundSnowClassNr1_df[0], groundSnowClassNr1_df[1])
#elevNr1 = groundSnowClassNr1_df[2]

#from mpl_toolkits.basemap import Basemap
#fig = plt.figure(figsize=(10, 8))
#m = Basemap(projection='lcc', resolution='c', width=8E6, height=8E6, lat_0=45, lon_0=-100,)
#m.shadedrelief(scale=0.5)
#m.pcolormesh(lonNr1, latNr1, elevNr1, latlon=True, cmap='RdBu_r')
#plt.clim(-8, 8)
#m.drawcoastlines(color='lightgray')
#
#plt.title('snow on the ground NR1')
#plt.colorbar(label='snow depth');
#%%groundSnowClassNr1_df[groundSnowClassNr1_df.columns[1:3]]   #plt.contourf   cmap=cm.RdBu,
fig = plt.figure(figsize = (12, 8))
ax = fig.add_subplot(111)
Z = np.ma.array(groundSnowClassNr1_df[2])

p = ax.pcolor(groundSnowClassNr1_df[0], groundSnowClassNr1_df[0], Z, vmin=abs(Z).min(), vmax=abs(Z).max())
cb = fig.colorbar(p)

# levels = list(np.arange(0.15, 1.5, 0.1)))
plt.title("Elevation Contours of Mt. Shasta")
plt.gca().set_aspect('equal', adjustable='box')
plt.show()

#plt.imshow(elevation, cmap='gist_earth', extent=extentNR1)
#plt.savefig("H:/Chapter2_snow_forest/LIDAR_analysis/Niwot/elev_.png")
