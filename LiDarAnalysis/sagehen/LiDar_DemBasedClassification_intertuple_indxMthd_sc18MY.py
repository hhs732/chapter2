...
def readLasFile (lasFilePath):
    infile = ls.file.File(lasFilePath, mode="r")
    unasigned = infile.Classification == 1
    ground = infile.Classification == 2
    lveg = infile.Classification == 3
    mveg = infile.Classification == 4
    hveg = infile.Classification == 5

    coords_uas = np.vstack((infile.x[unasigned], infile.y[unasigned], infile.z[unasigned])).T
    coords_grnd = np.vstack((infile.x[ground], infile.y[ground], infile.z[ground])).T
    coords_lveg = np.vstack((infile.x[lveg], infile.y[lveg], infile.z[lveg])).T
    coords_mveg = np.vstack((infile.x[mveg], infile.y[mveg], infile.z[mveg])).T
    coords_hveg = np.vstack((infile.x[hveg], infile.y[hveg], infile.z[hveg])).T
    coords = np.vstack((coords_uas,coords_grnd,coords_lveg,coords_mveg,coords_hveg))
...
pathNameS0fvcm = "C:/1UNRuniversityFolder/Dissertation/Chapter 2-snow-forest/LiDarAnalysis/sagehen/dem2014snow0ff_G.png"
elevationVegvcm = readPlotDEM(filenameS0fvcm,elevationMissNoS0f,pathNameS0fvcm)#

elevationMissNo = 1800
demset = gdal.Open(filenameS0fvcm)
band = demset.GetRasterBand(1)
elevation = band.ReadAsArray()
elevation[elevation == -9999.] = elevationMissNo
elevation[elevation > 10000.] = elevationMissNo
...
filename_slope_p = "C:/1UNRuniversityFolder/Dissertation/Chapter 2-snow-forest/LiDarAnalysis/sagehen/slopeSc_P.png"
slope_degree = readPlotDEM(filename_slope,0,filename_slope_p)
...
coordsVegVcm_df = pd.concat([coordsVegVcm_df1])#,coordsVegVcm_df2,coordsVegVcm_df1,coordsVegVcm_df3])
...
minXVcm = np.max([np.min(dem_groundPointsVegVcm_df['x']),np.min(coordsSnwVcm_df['x']),np.min(coordsVegVcm_df['x'])])+3000
maxXVcm = minXVcm+100#np.min([np.max(dem_groundPointsVegVcm_df['x']),np.max(coordsSnwVcm_df['x']),np.max(coordsVegVcm_df['x'])])
minYVcm = np.max([np.min(dem_groundPointsVegVcm_df['y']),np.min(coordsSnwVcm_df['y']),np.min(coordsVegVcm_df['y'])])+2500#3971000-1500
maxYVcm = minYVcm+150#np.min([np.max(dem_groundPointsVegVcm_df['y']),np.max(coordsSnwVcm_df['y']),np.max(coordsVegVcm_df['y'])])
...
#path2npoutVeg="G:/hhs_DST/lidar_analysis/sagehen/upGroundPointsVegSc18MY.npy"
#np.save(path2npoutVeg,upGroundPointsVegVcm)   
#upLoadUpGroundPointsVeg=np.load(path2npoutVeg)
#
#path2npoutSnw="G:/hhs_DST/lidar_analysis/sagehen/upGroundPointsSnwSc18MY.npy"
...
#path4npoutSnw="G:/hhs_DST/lidar_analysis/sagehen/allTreeClassNoLowVegSc18MY.npy"
...
#path5npoutSnw="G:/hhs_DST/lidar_analysis/sagehen/all0penClassNoGrassSc18MY.npy"
...
#path6npoutSnw="G:/hhs_DST/lidar_analysis/sagehen/allSnowNoLowVegClassSc18MY.npy"
...
#path7npoutSnw="G:/hhs_DST/lidar_analysis/sagehen/allsnowUnderTreeClassSc18MY.npy"
...
#path8npoutSnw="G:/hhs_DST/lidar_analysis/sagehen/allNoSnowUnderTreeClassSc18MY.npy"
...
#path9npoutSnw="G:/hhs_DST/lidar_analysis/sagehen/allSnow0penGroundClassSc18MY.npy"
...
#path10npoutSnw="G:/hhs_DST/lidar_analysis/sagehen/allNoSnow0penClassSc18MY.npy"
...
plt.savefig('G:/hhs_DST/lidar_analysis/sagehen/fSCA_pixel_sagehen_elev_topo15_noG30MY181.png')
