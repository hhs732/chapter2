...
#coordsVegVcm_df['z'] = coordsVegVcm_df['z']-0.38
...
maxXVcm = minXVcm + 1000.
minYVcm = np.max([np.min(dem_groundPointsVegVcm_df['y']),np.min(coordsSnwVcm_df['y']),np.min(coordsVegVcm_df['y'])])
#maxYVcm = np.min([np.max(dem_groundPointsVegVcm_df['y']),np.max(coordsSnwVcm_df['y']),np.max(coordsVegVcm_df['y'])])#minYVcm+2000
maxYVcm = minYVcm + 1200.
...
allTreeReturnVcm_df = (pd.DataFrame(defineSpecificClassGreater (vegClassVcm_df, 2))).astype(int)
...
allTreeClassVcm_df = (vegClassVcm_df.loc[allTreeReturnVcm_indx['Index']]).astype(int)
...
allsnowUnderTreeClassVcm_df = (pd.DataFrame(defineSpecificClassGreater (allsnowUnderTreeClassVcm_df0[['x','y','z']], 0.15))).astype(int)
...
allNoSnowUnderTreeClassVcm_df = (pd.DataFrame(defineSpecificClassGreater (allNoSnowUnderTreeClassVcm_df0[['x','y','z']], -0.3))).astype(int)
...
test = len(allsnowUnderTreeClassVcm_df0[['x','y']].drop_duplicates(keep='first'))
UT_NoSnowPixels = len(allNoSnowUnderTreeClassVcm_df0[['x','y']].drop_duplicates(keep='first'))
UT_fSCA = 100.*(test-UT_NoSnowPixels)/test#(UT_snowPixels+UT_NoSnowPixels)
...
d11 = [0,0,0,FSCA_0p_em,FSCA_0p_nm,FSCA_0p_sm,FSCA_0p_eh,FSCA_0p_nh,FSCA_0p_sh]

x22 = ['exp_low_UT','flt_low_UT','shl_low_UT','exp_mid_UT',
      'flt_mid_UT','shl_mid_UT','exp_hi_UT','flt_hi_UT','shl_hi_UT']
d22 = [0,0,0,FSCA_ut_em,FSCA_ut_nm,FSCA_ut_sm,FSCA_ut_eh,FSCA_ut_nh,FSCA_ut_sh]
# create plot
...
rects1 = plt.bar(index, d11, bar_width,color='wheat',edgecolor='brown',label='0pen')
