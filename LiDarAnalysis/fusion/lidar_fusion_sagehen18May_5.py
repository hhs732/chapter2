...
def readPlotDEM(filename,elevationMissNo,pathName,extent2):#extent2 = [indx0=4364802,indx1,col0=730002,col1]
    ...
    elevation_cut = elevation_df.loc[extent2[0]:extent2[1]-1,extent2[2]:extent2[3]-1] #4364802  730002
...
def creatingCentroidGroundpointsFromDem(tiffFilename,elevationMissNo,extent2):#,pathNameforDemImage):
    ...
    elevation_cut = elevation_df.loc[extent2[0]:extent2[1]-1,extent2[2]:extent2[3]-1]

    nrows2, ncols2 = elevation_cut.values.shape
  
    latitude = np.arange(extent2[2],extent2[3],dtype=int)
    longitude = np.arange(extent2[0],extent2[1],dtype=int)
...
def creatingLatituteLongitudeFromDem(tiffFilename,elevationMissNo,extent2):#,pathNameforDemImage):
    demset = gdal.Open(filenameIn)
    band = demset.GetRasterBand(1)
    elevation = band.ReadAsArray()
    elevation[elevation == -9999.] = elevationMissNoS0f
    ...
    return latitude_rp, longitude_rp, nrows2, ncols2
...
def calculatingSlopeAspectNorthness(demFilename,slopeFileName,aspectFileName,elevationMissNoS0f):
    ...
    #elevation_df = pd.DataFrame(elevation, columns = column, index = indx)
    #elevation_cut = elevation_df.loc[extent2[0]:extent2[1]-1,extent2[2]:extent2[3]-1]
m   #nrows2, ncols2 = elevation_cut.values.shape
    #latitude = np.arange(extent2[2],extent2[3],dtype=int)
    #longitude = np.arange(extent2[0],extent2[1],dtype=int)
...
extent2 = [4364802,4372274,730236,737000]
...
dem_groundPoints, dem_groundPointsVeg_df = creatingCentroidGroundpointsFromDem(filenameIn,elevationMissNoS0f,extent2)#,pathNameS)
lat, longt, nrows, ncols = creatingLatituteLongitudeFromDem(filenameIn,elevationMissNoS0f,extent2)
