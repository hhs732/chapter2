...
elevationVeg_count = readPlotDEM(veg_count_path,-4,veg_count_path_out)#
veg_count_lst, veg_count_df = creatingCentroidGroundpointsFromDem(veg_count_path,-4)
...
elevationVeg_totalCount = readPlotDEM(veg_totalCount_path,0,veg_totalCount_path_out)#
veg_totalCount_lst, veg_totalCount_df = creatingCentroidGroundpointsFromDem(veg_totalCount_path,0)
...
elevationVeg_allCoverCount = readPlotDEM(veg_allCoverCount_path,0,veg_allCoverCount_path_out)#
veg_allCoverCount_lst, veg_allCoverCount_df = creatingCentroidGroundpointsFromDem(veg_allCoverCount_path,0)

#cutting some part of maps
minXVcm = np.max([np.min(dem_groundPointsVeg_df['x']),np.min(veg_allCoverCount_df['x']),np.min(veg_totalCount_df['x']),np.min(veg_count_df['x']),731000])#+2000
maxXVcm = np.min([np.max(dem_groundPointsVeg_df['x']),np.max(veg_allCoverCount_df['x']),np.max(veg_totalCount_df['x']),np.max(veg_count_df['x'])])#minXVcm+500
minYVcm = np.max([np.min(dem_groundPointsVeg_df['y']),4365000])#+1500
maxYVcm = np.min([np.max(dem_groundPointsVeg_df['y']),4372000])#minYVcm+510

extent=[minXVcm, maxXVcm, minYVcm, maxYVcm]
plt.figure(figsize=(30,20))
...
dem_Veg_intg = cutPart0fMapSort(dem_groundPointsVeg_df,minXVcm,maxXVcm,minYVcm,maxYVcm)
veg_count_intg = cutPart0fMapSort(veg_count_df,minXVcm,maxXVcm,minYVcm,maxYVcm)
veg_totalCount_intg = cutPart0fMapSort(veg_totalCount_df,minXVcm,maxXVcm,minYVcm,maxYVcm)
veg_allCoverCount_intg = cutPart0fMapSort(veg_allCoverCount_df,minXVcm,maxXVcm,minYVcm,maxYVcm)
index_cut = cutPart0fMap(index_noG30,minXVcm,maxXVcm,minYVcm,maxYVcm)

newNcols = maxYVcm - minYVcm + 1
#calculating topo_dimension from cut northness
params = [-0.1,0.1,2000,2300]
topo_dimension_vcm = classificationNorthnessElev(index_cut.values,params[0],params[1],params[2],params[3],newNcols,minXVcm,minYVcm)
