...
def readPlotDEM(filename,elevationMissNo,pathName,extent2):#extent2 = [indx0=4364802,indx1,col0=730002,col1]
    ...
    column = np.arange(x0,x1,dtype=int)
    indx = np.arange(y1,y0,dtype=int)
    elevation_df = pd.DataFrame(elevation, columns = column, index = indx)
    
    elevation_cut = elevation_df.loc[extent2[2]:extent2[3]-1,extent2[0]:extent2[1]-1] #4364802  730002
    
    plt.figure(figsize=(30,20))
    plt.imshow(elevation_cut.values, cmap='gist_earth', extent=extent2)
    plt.savefig(pathName)
    
    return elevation, elevation_cut

def creatingCentroidGroundpointsFromDem(tiffFilename,elevationMissNo,extent2):#,pathNameforDemImage):
    ...
    elevation_cut = elevation_df.loc[extent2[2]:extent2[3]-1,extent2[0]:extent2[1]-1]

    nrows2, ncols2 = elevation_cut.values.shape
  
    latitude = np.arange(extent2[0],extent2[1],dtype=int)
    longitude = np.arange(extent2[2],extent2[3],dtype=int)
    
    latitude_rp = (np.tile(latitude, nrows2)).astype(int)
    longitude_rp = (np.repeat(longitude, ncols2)).astype(int)
    elevation_rp = np.reshape(elevation_cut.values,(nrows2*ncols2)).T
...
extent2 = [730240,738000,4365000,4370000]
...
elevationVeg, elevationVeg_cut = readPlotDEM(filenameIn,elevationMissNoS0f,pathNameOut,extent2)#
dem_groundPointsVeg_df = creatingCentroidGroundpointsFromDem(filenameIn,elevationMissNoS0f,extent2)#,pathNameS)
...
index_df_cut = cutPart0fASCIISort(index_colNm_df,extent2[0],extent2[1]-1,extent2[2],extent2[3]-1)
...
elevationVeg_count, elevationVeg_count_cut = readPlotDEM(veg_count_path,-4,veg_count_path_out,extent2)#,2000,7000,1000,11000
veg_count_df = creatingCentroidGroundpointsFromDem(veg_count_path,0,extent2)
...
elevationVeg_totalCount, elevationVeg_totalCount_cut = readPlotDEM(veg_totalCount_path,0,veg_totalCount_path_out)#,2000,7000,1000,11000)#
veg_totalCount_df = creatingCentroidGroundpointsFromDem(veg_totalCount_path,0,extent2)
...
elevationVeg_allCoverCount, elevationVeg_allCoverCount_cut = readPlotDEM(veg_allCoverCount_path,0,veg_allCoverCount_path_out)#,2000,7000,1000,11000)#
veg_allCoverCount_df = creatingCentroidGroundpointsFromDem(veg_allCoverCount_path,0,extent2)
...
elevationsnow_count3, elevationsnow_count3_cut = readPlotDEM(snow_count_path,0,snow_count_path_out)#,1534,5500,1593,6508
snow_count_df = creatingCentroidGroundpointsFromDem(snow_count_path,0,extent2)
...
elevationsnow_totalCount3, elevationsnow_totalCount3_cut = readPlotDEM(snow_totalCount_path,0,snow_totalCount_path_out)#,1534,5500,1593,6508
snow_totalCount_df = creatingCentroidGroundpointsFromDem(snow_totalCount_path,0,extent2)
...
elevationsnow_allCoverCount3, elevationsnow_allCoverCount3_cut = readPlotDEM(snow_allCoverCount_path,0,snow_allCoverCount_path_out)#,1534,5500,1593,6508
snow_allCoverCount_df = creatingCentroidGroundpointsFromDem(snow_allCoverCount_path,0,extent2)
