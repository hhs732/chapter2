...
def fSCAcalculation_elevNorthRandom(veg_snow_temp_elev_clsf_file,elev_class_inx,sample_size): #veg_snow_temp_elev_clsf_file[:,8]==101 contains elevation classification
    ...
    #fSCA_unTr_exp = {0: 0, 1: 1, 2: 2, 3: 2, 4: 2, 5: 2, 6: 2, 7: 2, 8: 2, 9: 2} 
    #fSCA_unTr_shl = {0: 0, 1: 1, 2: 2, 3: 2, 4: 2, 5: 2, 6: 2, 7: 2, 8: 2, 9: 2}
    #fSCA_0pen_exp = {0: 0, 1: 1, 2: 2, 3: 2, 4: 2, 5: 2, 6: 2, 7: 2, 8: 2, 9: 2}
    #fSCA_0pen_shl = {0: 0, 1: 1, 2: 2, 3: 2, 4: 2, 5: 2, 6: 2, 7: 2, 8: 2, 9: 2}
    fSCA_0u_exp = {0: 0, 1: 1, 2: 2, 3: 2, 4: 2, 5: 2, 6: 2, 7: 2, 8: 2, 9: 2} 
    fSCA_0u_shl = {0: 0, 1: 1, 2: 2, 3: 2, 4: 2, 5: 2, 6: 2, 7: 2, 8: 2, 9: 2}
    
    for indx in range (len(elev_class_inx)): 
        
        #fSCA_unTr_exp_rand = np.zeros(sample_size) 
        #fSCA_unTr_shl_rand = np.zeros(sample_size)
        #fSCA_0pen_exp_rand = np.zeros(sample_size)
        #fSCA_0pen_shl_rand = np.zeros(sample_size)
        ...
        for rand in range (sample_size):
            ...
            #fSCA_unTr_exp_rand[rand] = fSCA_ut_exp
            #fSCA_unTr_shl_rand[rand] = fSCA_ut_shl
            ...
            #fSCA_0pen_exp_rand[rand] = fSCA_0p_exp
            #fSCA_0pen_shl_rand[rand] = fSCA_0p_shl
        ...
        #fSCA_unTr_exp[indx] = fSCA_unTr_exp_rand
        #fSCA_unTr_shl[indx] = fSCA_unTr_shl_rand
        #fSCA_0pen_exp[indx] = fSCA_0pen_exp_rand
        #fSCA_0pen_shl[indx] = fSCA_0pen_shl_rand
...
fSCA_0u_exp_nrc_samp, fSCA_0u_shl_nrc_samp = fSCAcalculation_elevNorthRandom(veg_snow_temp_clsfnrc,elev_class_inx,100)
...
average_sample_exp = []
for iii in range (10):
    statistic_nrc_each, pvalue_nrc_each = scipy.stats.wilcoxon(fSCA_0u_exp_nrc_samp[iii][0:10], fSCA_0u_shl_nrc_samp[iii][0:10], zero_method='wilcox', correction=False)
    statistic_nrc.append(statistic_nrc_each)
    pvalue_nrc.append(pvalue_nrc_each)
    average_sample_shl.append(np.average(fSCA_0u_shl_nrc_samp[iii]))
    average_sample_exp.append(np.average(fSCA_0u_exp_nrc_samp[iii]))
