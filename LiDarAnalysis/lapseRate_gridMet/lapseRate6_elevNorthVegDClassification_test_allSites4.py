...
#np.save('C:/1UNRuniversityFolder/Dissertation/Chapter 2-snow-forest/LiDarAnalysis/lapseRate_gridMet/snow_temp_vegdens_index_sL30_nrc',snow_temp_vegdens_index_sL30_nrc)
...
aaaa_test1 = snow_temp_vegdens_index_sL30_nrc[0:710,:]

elev_nrc = snow_temp_vegdens_index_sL30_nrc[:,2]
tempDJF_gm_nrc = -0.007512*elev_nrc + 14.168475

#%%#calculating topo_dimension from cut northness
lat_long_nad83_nrc = [np.min(snow_temp_vegdens_index_sL30_nrc[:,0]),np.max(snow_temp_vegdens_index_sL30_nrc[:,0]),np.min(snow_temp_vegdens_index_sL30_nrc[:,1]),np.max(snow_temp_vegdens_index_sL30_nrc[:,1])]
lat_long_degre_nrc = [39.978183, 40.025319, -105.58556840332032, -105.52620245393197]

ncols = 455100-450000+1
lat_nrc =[]
for dx in range (ncols):
    lat_nrc.append(450000+dx)

nrows = 4430700-4425500+1
lon_nrc = []
...
nn=0
indx_snwPres = np.array(snwPres_df_nrc.index)
snowMap[indx_snwPres] = sm[indx_snwPres[:]]
for ind in indx_snwPres:
    if nn%100000==0:
